...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const morphology={
      ...
      bellyDepthRatio: Math.min(0.9,Math.max(0.35, +(0.50 + ((seed>>3)%18)/100).toFixed(2))),
      hockAngleDeg: Math.min(170,Math.max(120, 138 + ((seed>>5)%18))),
      toplineDeviation: Math.min(0.3,Math.max(0, +(0.04 + ((seed>>7)%9)/100).toFixed(2))),
    ...
    if(morphology.hockAngleDeg>=140) strengths.push('Ángulo de corvejón funcional (buena locomoción)');
    if(morphology.bellyDepthRatio>=0.50 && morphology.bellyDepthRatio<=0.68) strengths.push('Capacidad ruminal adecuada sin panza exagerada');
    if(morphology.bodyLenToHeight>=1.55) strengths.push('Longitud corporal favorable para rendimiento cárnico');
    if(morphology.hockAngleDeg<130) weaknesses.push('Corvejón cerrado (riesgo locomotor)');
    if(morphology.toplineDeviation>0.18) weaknesses.push('Dorso cóncavo (posible menor eficiencia)');
    if(morphology.bellyDepthRatio>0.72) weaknesses.push('Abdomen muy profundo (riesgo de baja conversión)');
