...
function poseReliable(posture){ try{ if(!posture) return false; const rotated = Number.isFinite(posture.rotationDeg) && posture.rotationDeg>15; const notSide = typeof posture.view==='string' && !/side|lateral/i.test(posture.view); const occluded = posture.occlusion===true || posture.truncation===true || posture.blur===true || posture.shadowStrong===true; return !(rotated||notSide||occluded); }catch{ return false; } }
function suspiciousBins(m){ let c=0; if(Number.isFinite(m?.hockAngleDeg) && m.hockAngleDeg>=128 && m.hockAngleDeg<=132) c++; if(Number.isFinite(m?.toplineDeviation) && m.toplineDeviation>=0.28 && m.toplineDeviation<=0.32) c++; if(Number.isFinite(m?.rumpSlope) && m.rumpSlope>=0.18 && m.rumpSlope<=0.22) c++; if(Number.isFinite(m?.bellyDepthRatio) && m.bellyDepthRatio>=0.48 && m.bellyDepthRatio<=0.52) c++; if(Number.isFinite(m?.bodyLenToHeight) && Math.abs(m.bodyLenToHeight-1.50)<=0.03) c++; return c; }
function flagsFromMetrics({hockAngleDeg,toplineDeviation,rumpSlope},bcs,category,ageMonths,posture){ const flags=[];
  const young = (category==='ternero' || category==='novillo') && Number.isFinite(ageMonths) && ageMonths<=18;
  const closedTh = young ? 130 : 132;
  const watchTh  = young ? 140 : 142;
  const reliable = poseReliable(posture);
  if(Number.isFinite(hockAngleDeg)){ if(hockAngleDeg<closedTh) flags.push('hock_angle_closed'); else if(hockAngleDeg<watchTh && reliable) flags.push('hock_angle_watch'); }
...
async function openaiJSON(key, model, messages){
  ...
  try{
    const r=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${key}`},body:JSON.stringify({model,temperature:0.1,response_format:{type:'json_object'},messages}),signal:controller.signal});
    const txt=await r.text(); if(!r.ok) throw new Error('OpenAI '+r.status+': '+txt.slice(0,300));
    let data; try{ data=JSON.parse(txt); }catch{ throw new Error('Parse OpenAI: '+txt.slice(0,200)); }
    let out; try{ out=JSON.parse(data?.choices?.[0]?.message?.content||'{}'); }catch{ out=null; }
    if(!out || typeof out!=='object') throw new Error('Empty JSON content');
    return out;
...
export default async function handler(req,res){
  ...
  try{
    ...
    // Posible refinado de corvejón/topline
    ...
    try{
      if( (defaultsSuspect===true) || (reliablePose && Number.isFinite(morph.hockAngleDeg) && morph.hockAngleDeg<135) ){
        const sysR = "Eres medidor pericial de morfología. Re-ESTIMA desde la IMAGEN (solo desde pixeles) con máxima precisión: 'hockAngleDeg' y 'toplineDeviation'. Evita valores genéricos 130, 0.30. Devuelve SOLO JSON {hockAngleDeg:number, toplineDeviation:number}";
        let outR=null; try{ outR = await openaiJSON(key, model, [
    ...
    const prelimScore = scoreFattening({metrics:morph,bcs,sex,ageMonths:ageGuessMonths,category:categoryGuess,posture});
    ...
    auditText = (auditText||'') + (refinedNote? ' '+refinedNote : '') + (defaultsSuspect? ' · detección anti-defaults' : '');
    const json = {
      source:'openai',
      auditPass:true,
      morphology:morph,
